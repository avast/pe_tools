...
from .rsrc import (
    parse_pe_resources,
    pe_resources_prepack,
    parse_prelink_resources,
    KnownResourceTypes,
)
...
class Version:
    def __init__(self, s):
        ...
        if (
            not self._parts
            or len(self._parts) > 4
            or any(part < 0 or part >= 2**16 for part in self._parts)
        ):
...
def main():
    ap = argparse.ArgumentParser(
        ...
        description="Parses and edits resources in Windows executable (PE) files.",
    )
    ap.add_argument(
        "--remove-signature",
        action="store_true",
        help="remove the signature. If the file contains one, editing the file will fail",
    )
    ap.add_argument(
        "--ignore-trailer",
        action="store_true",
        help="keep trailing data (typically in a setup program) intact, move them if necessary",
    )
    ap.add_argument(
        "--remove-trailer",
        action="store_true",
    ...
    ap.add_argument(
        "--update-checksum",
        action="store_true",
        help="set the correct checksum (can be slow on large files), zero it out otherwise",
    )
    ap.add_argument(
        "--clear",
        "-C",
    ...
    ap.add_argument(
        "--clear-manifest", action="store_true", help="remove the manifest resource"
    )
    ...
    gp.add_argument(
        ...
        metavar="DEP",
        default=[],
        help="add dependency. DEP should be a space separated list of key=value pairs, e.g. "
    ...
    gp.add_argument(
        ...
        action="append",
        metavar="STR",
        help='updates the specified version-info field, e.g. FileVersion="1, 2, 3, 4"',
    )
    gp.add_argument(
        "--set-resource",
        "-R",
        ...
        action="append",
        default=[],
        help='set a resource entry to the contents of a file, e.g. "-R RT_RCDATA prog.exe 0 prog.exe"',
    )

    ap.add_argument(
        "--output",
        "-o",
        help="write the edited contents to OUTPUT instead of editing the input file in-place",
    )
    ...
    if (
        not args.clear
        and not args.apply
        and not args.add_dependency
        and not args.set_version
        and not args.set_resource
    ):
        return 0
    ...
    if not pe.is_dir_safely_resizable(IMAGE_DIRECTORY_ENTRY_RESOURCE):
        print(
            "error: the resource section is not resizable: {}".format(args.file),
            file=sys.stderr,
        )
        return 3
        ...
        for resource_type in r:
            for name in r[resource_type]:
                for lang in r[resource_type][name]:
                    resources.setdefault(resource_type, {}).setdefault(name, {})[
                        lang
                    ] = r[resource_type][name][lang]
    ...
    for rtype, rname, lang, inname in args.set_resource:
        ...
        if rname.startswith("#"):
            ...
            rname = rname.upper()
        resources.setdefault(rtype, {}).setdefault(rname, {})[int(lang)] = (
            grope.wrap_io(res_fin)
        )
    ...
    if args.add_dependency:
        ...
        dependency_list = man_doc.getElementsByTagName("dependency")
        if not dependency_list:
            dependency = man_doc.createElement("dependency")
            man.appendChild(dependency)
        else:
            dependency = dependency_list[0]

        dependent_assembly_list = man_doc.getElementsByTagName("dependentAssembly")
        if not dependent_assembly_list:
            dependent_assembly = man_doc.createElement("dependentAssembly")
            dependency.appendChild(dependent_assembly)
        else:
            dependent_assembly = dependent_assembly_list[0]

        for dep in args.add_dependency:
            dep_elem = man_doc.createElement("assemblyIdentity")
            for tok in dep.split():
                k, v = tok.split("=", 1)
                dep_elem.setAttribute(k, v)
            dependent_assembly.appendChild(dep_elem)

        resources[RT_MANIFEST][man_name][man_lang] = b"\xef\xbb\xbf" + man_doc.toxml(
            encoding="utf-8"
        )
    ...
    if args.set_version:
        ...
        for param in args.set_version:
            ...
            if len(toks) != 2:
                print(
                    'error: version infos must be in the form "name=value"',
                    file=sys.stderr,
                )
                return 2
            name, value = toks
